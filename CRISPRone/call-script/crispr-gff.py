...
for suffix in [".plot.txt", ".suspicious"]: 
   ...
   for plotfile in plotfiles:
       ...
       if len(lines) < 1:
           continue
       #YY July 2021, to check split() -> split("\t")
       #there are antiCRISPR with blank coordinates
       tmps = lines[0].strip().split("\t")
       ...
       for aline in lines:
           #YY July 2021 split() -> split("\t")
           subs = aline.strip().split("\t") 
           if len(subs) < 6:
               continue
           (sid, spos, epos, strand, des, stype) = subs[0:6]
           if not (re.match("\d+", spos) and re.match("\d+", epos)): #no coordinates
               continue
           if "repeat" in des:
               tmp = stype.split(":")
